...
import { SpecificDaysInterface, DayOfWeek, RuleType, ChargeCodeResponseInterface } from '../model/create-rules.interface';
...
export class CreateRuleUtilityService {
  ...
  saveParamFramer(createRuleModel: CreateRuleModel, framerObject: object,
    averagingRulesModel: AveragingRulesModel, viewDocumentationModel: ViewDocumentationModel): object {
  ...
  saveParamFramerFree(createRuleModel: CreateRuleModel, framerObject: object,
    ...
    documentModel: ViewDocumentationModel, optionalModel) {
    ...
    if (freeRuleModel && freeEventTypeModel) {
      ...
      framerObject['accessorialFreeRuleTypes'].push(this.createFreeFuleParamEventType(freeRuleModel.freeRulesForm,
        freeEventTypeModel.freeEventTypeForm, createRuleModel));
    } else if (freeRuleModel && freeCalendarModel) {
      framerObject['accessorialFreeRuleTypes'] = [];
      framerObject['accessorialFreeRuleTypes'].
        push(this.createFreeRuleParamCalendar(freeRuleModel.freeRulesForm, freeCalendarModel.freeCalendarForm, createRuleModel));
    } else if (freeRuleModel) {
      framerObject['accessorialFreeRuleTypes'] = [];
      framerObject['accessorialFreeRuleTypes'].push(this.createFreeFuleParam(freeRuleModel.freeRulesForm, createRuleModel));
    }
    return framerObject;
  }
  ...
  setAccessorialRuleValues(parentScope, editResponse) {
    parentScope.createRuleModel.editAccessorialWholeResponse = editResponse.editRuleData;
    parentScope.createRuleModel.ruleConfigurationId = editResponse.ruleConfigurationId;
    parentScope.createRuleModel.isEditFlagEnabled = true;
    parentScope.createRuleModel.refreshDocumentResponse = editResponse.refreshDocumentResponse;
    parentScope.createRuleModel.editAccessorialWholeResponse.requestServices = editResponse['editRequestedService'];
    parentScope.createRuleModel.breadCrumbList[3] = {
      label: 'Edit Accessorial Rule Setup'
    };
    parentScope.patchValuesToAccessorialRules(editResponse.editRuleData);
  }
  ...
  createFreeFuleParam(freeRuleModel: FormGroup, createRuleModel: CreateRuleModel) {
    ...
    if (createRuleModel.editAccessorialWholeResponse
      && createRuleModel.editAccessorialWholeResponse.accessorialFreeRuleQuantityTypes) {
      accessorialFreeRuleType['accessorialFreeRuleQuantityType']['accessorialFreeRuleQuantityId'] =
        createRuleModel.editAccessorialWholeResponse['accessorialFreeRuleQuantityTypes']
        ['accessorialFreeRuleQuantityId'];
    }
    accessorialFreeRuleType['accessorialFreeRuleQuantityType']
  ...
  createFreeRuleParamCalendar(freeRulesForm: FormGroup, freeCalendarForm: FormGroup, createRuleModel: CreateRuleModel) {
    const accessorialFreeRuleType = {};
    ...
    if (createRuleModel.editAccessorialWholeResponse
      && createRuleModel.editAccessorialWholeResponse.accessorialFreeRuleCalenderTypes) {
      accessorialFreeRuleType['customerAccessorialFreeRuleCalendar']['customerAccessorialFreeRuleCalendarId'] =
        createRuleModel.editAccessorialWholeResponse['accessorialFreeRuleCalenderTypes']
        ['customerAccessorialFreeRuleCalendarId'];
    }
    accessorialFreeRuleType['customerAccessorialFreeRuleCalendar']
    ...
    accessorialFreeRuleType['customerAccessorialFreeRuleCalendar']
    ['customerAccessorialFreeRuleCalendarWeekDay'] = this.getFreeRuleDayOfWeek(freeCalendarForm, createRuleModel);
    accessorialFreeRuleType['customerAccessorialFreeRuleCalendar']
    ...
    ['customerAccessorialFreeRuleCalendarMonth'] = [];
     if (timeFrameValue !== 'Weekly') {
      accessorialFreeRuleType['customerAccessorialFreeRuleCalendar']['customerAccessorialFreeRuleCalendarMonth'].push({
        ...
        'customerAccessorialFreeRuleCalendarMonthId': null
      });
    }
    return accessorialFreeRuleType;
  }
  ...
  getFreeRuleDayOfWeek(freeCalendarForm: FormGroup, createRuleModel: CreateRuleModel) {
      ...
      dayOfWeekList.forEach(dayOfWeek => {
        if (dayOfWeek) {
          const dayOfWeekParam = {
            ...
            customerAccessorialFreeRuleCalendarWeekDayId: null
          };
  ...
  createFreeFuleParamEventType(freeRuleModel: FormGroup, freeEventTypeModel: FormGroup, createRuleModel: CreateRuleModel) {
    ...
    if (createRuleModel.editAccessorialWholeResponse && createRuleModel.editAccessorialWholeResponse.accessorialFreeRuleEventTypes) {
      accessorialFreeRuleEventType['customerAccessorialFreeRuleEvent']['customerAccessorialFreeRuleEventId'] =
        createRuleModel.editAccessorialWholeResponse['accessorialFreeRuleEventTypes']['customerAccessorialFreeRuleEventId'];
    }
    accessorialFreeRuleEventType['customerAccessorialFreeRuleEvent']
    ...
    accessorialFreeRuleEventType['customerAccessorialFreeRuleEvent']
    ['accessorialDayAfterEventFreeRuleModifierName'] = this.getFormControlLabel(freeEventTypeModel, 'freeAmountSecondEvent');
  ...
  populateRuleType(ruleType: RuleType, createRuleModel: CreateRuleModel, changeDetector) {
    if (ruleType) {
      ...
      createRuleModel.ruleType = ruleTypeValues;
      changeDetector.detectChanges();
    }
  }
  ...
  setAlternateChargeTypesForEdit(parentScope) {
    if (parentScope.createRuleModel.isEditFlagEnabled) {
  ...
  setFormErrors(parentScope: CreateRulesComponent, changeDetector: ChangeDetectorRef) {
  ...
  saveAndCreateNewFreeRule(parentScope) {
    parentScope.createRuleModel.isShowSavePopup = false;
    parentScope.createRuleModel.selectedFreeRuleType = null;
    const freeTypeValue = parentScope.freeRuleTab.freeRuleModel.freeRulesForm.controls['freeRuleType'] &&
      parentScope.freeRuleTab.freeRuleModel.freeRulesForm.controls['freeRuleType'].value ?
      parentScope.freeRuleTab.freeRuleModel.freeRulesForm.controls['freeRuleType'].value : null;
    this.clearFreeRuleFormOnSaveAndCreateNew(parentScope.freeRuleTab, freeTypeValue);
    parentScope.optionalFields.optionalAttributesModel.optionalForm.controls.businessUnit.setValue(null);
    parentScope.optionalFields.optionalAttributesModel.optionalForm.controls.serviceLevel.reset();
    parentScope.optionalFields.optionalAttributesModel.optionalForm.controls.serviceLevel.clearValidators();
    parentScope.optionalFields.optionalAttributesModel.optionalForm.controls.serviceLevel.updateValueAndValidity();
    parentScope.optionalFields.optionalAttributesModel.optionalForm.controls.requestedService.reset();
    parentScope.optionalFields.optionalAttributesModel.serviceLevel = [];
    parentScope.optionalFields.optionalAttributesModel.operationalService = [];
    parentScope.billTo['billToModel']['selectedBillTo'] = [];
    parentScope.billTo.changeDetector.detectChanges();
    parentScope.optionalFields['optionalAttributesModel']['optionalForm'].reset();
    parentScope.createRuleModel.rulesForm.controls['ruleLevel'].setValue('Agreement');
    parentScope.createRuleModel.selectedRuleLevel = 'Agreement';
    parentScope.contractSectionReset();
    parentScope.removeDocumentation();
    parentScope.ratesFieldReset();
    parentScope.createRuleModel.isFreeRuleSaveAndClose = true;
  }
  createReferenceGridParams(parentScope) {
    let referenceGridParams = {};
    let freeEventTypeModel = null, freeCalendarModel = null, freeRuleModel = null;
    const ruleTypeLabel = parentScope.createRuleModel.rulesForm.controls.ruleType &&
      parentScope.createRuleModel.rulesForm.controls.ruleType.value ?
      parentScope.createRuleModel.rulesForm.controls.ruleType.value.label : null;
    const isEmptyFormFields = ['requestServiceDTOs', 'carrierDTOs', 'businessUnitServiceOfferingDTOs'];
    if (parentScope.freeRuleTab) {
      freeEventTypeModel = parentScope.freeRuleTab.freeTypeEventTab ? parentScope.freeRuleTab.freeTypeEventTab.freeEventTypeModel : null;
      freeCalendarModel = parentScope.freeRuleTab.freeTypeCalendarTab ?
        parentScope.freeRuleTab.freeTypeCalendarTab.freeCalendarModel : null;
      freeRuleModel = parentScope.freeRuleTab.freeRuleModel ? parentScope.freeRuleTab.freeRuleModel : null;
    }
    if (ruleTypeLabel && ruleTypeLabel.toLowerCase() === 'free' && parentScope.createRuleModel.isFreeRuleSaveAndClose) {
      referenceGridParams = parentScope.createRateUtilityService.onRefreshRatePostFramer(parentScope.createRuleModel,
        parentScope.createRuleModel.rulesForm, parentScope.optionalFields['optionalAttributesModel'],
        parentScope.billTo['billToModel'], parentScope.agreementID);
      ...
      referenceGridParams = parentScope.createRuleUtilityService.saveParamFramerFree(parentScope.createRuleModel,
        referenceGridParams, freeRuleModel, freeEventTypeModel, freeCalendarModel,
        parentScope.documentation.viewDocumentationModel, parentScope.optionalFields['optionalAttributesModel']);
      parentScope.referenceTable.referenceModel.referenceGridRequest = referenceGridParams;
      parentScope.createRuleModel.showReferenceGrid = referenceGridParams;
    }
  }
