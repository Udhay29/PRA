...
const yearValidator: ValidatorFn = (fg: FormGroup) => {
  ...
  dayOfMonthValue = dayOfMonthControl && dayOfMonthControl['value'] ? dayOfMonthControl['value']['value'] : null;
...
export class FreeCalendarComponent implements OnInit {
  ...
  patchValuesForEdit() {
    const dayOfMonthControl = this.freeCalendarModel.freeCalendarForm.controls['dayOfMonth'];
    const monthControl = this.freeCalendarModel.freeCalendarForm.controls['months'];
    if (dayOfMonthControl && dayOfMonthControl.value) {
      const selectedMonth = utils.find( this.freeCalendarModel.months ,
         { 'label':  monthControl.value.label });
         monthControl.setValue(selectedMonth);
         const dayofMonthSelectedValue = dayOfMonthControl.value;
         this.onSelectMonth(selectedMonth['value']);
         dayOfMonthControl.setValue(dayofMonthSelectedValue);
     }
  }
  ...
  setApplyIfDefault() {
    if (this.freeCalendarModel.freeCalendarForm.controls['applyIf']) {
      const applyIfSelectedValue = {
        value: this.freeCalendarModel.applyIfDayAlways['accessorialFreeRuleCalendarApplyTypeId'],
        label: this.freeCalendarModel.applyIfDayAlways['accessorialFreeRuleCalendarApplyName']
      };
      this.freeCalendarModel.freeCalendarForm.patchValue({
