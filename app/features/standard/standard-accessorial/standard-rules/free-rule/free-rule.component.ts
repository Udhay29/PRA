...
import { CreateRuleUtilityService } from '../create-rules/service/standard-create-rule-utility.service';
...
populateQuanityType(quantityType) {
  ...
  addQuantityTypeControl() {
    ...
    this.freeRuleModel.quantityTypes.forEach((element) => {
      if (element['value'] === this.freeRuleModel.timeQuantityType['accessorialFreeRuleQuantityTypeId']) {
        this.freeRuleModel.freeRulesForm.patchValue({
          quantityType: element
        });
      }
    });
  ...
  populateDistanceType(distanceType) {
    if (distanceType) {
      ...
      this.addDistanceTypeControl();
      this.changeDetector.detectChanges();
    }
  }
  addDistanceTypeControl() {
